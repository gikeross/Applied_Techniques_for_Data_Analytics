...
select date, count(date) from (select * from covid_project.CovidDeaths where location = 'Italy') as sub1
group by 1
having 2 >1;

-- 1) Getting incidence of death over people who contracted the virus
select Location, date, total_cases, total_deaths, round((total_deaths/total_cases)*100,5) as DeathPercentage from covid_project.CovidDeaths
where location = 'Italy'
order by 5 desc; -- result higher & is 2020-06-20 

-- 2) Getting incidence of death over people who contracted the virus
select Location, date, total_cases, population, round((total_cases/population)*100,6) as cases_over_population from covid_project.CovidDeaths
where location = 'Italy'
order by 2 asc;
...
join covid_project.CovidDeaths vac
